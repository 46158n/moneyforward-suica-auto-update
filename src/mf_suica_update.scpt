...
-- 「電子マネー・プリペイド > モバイルSuica」の「更新」リンクをクリックし、
-- 続けて「登録金融機関」の一括更新ボタンもクリックする
...
"
        ...
        set bulkResultText to "skipped"

        if resultText is "clicked" then
            ------------------------------------------------------------------
            -- 6. Suica更新の処理が走り始めるまで少し待つ
            ------------------------------------------------------------------
            delay 5

            ------------------------------------------------------------------
            -- 7. 「登録金融機関」の一括更新ボタンをクリックするJS
            ------------------------------------------------------------------
            set bulkJsCode to "
(function() {
  try {
    // 「登録金融機関」セクション内の一括更新ボタン（.regist-action a.refresh）を探す
    const accountsSection = document.querySelector('section#registered-accounts');
    if (!accountsSection) return 'no-registered-accounts-section';

    const bulkUpdateButton = accountsSection.querySelector('.regist-action a.refresh');
    if (!bulkUpdateButton) return 'bulk-update-not-found';
...
"
            ...
            tell application "Google Chrome"
                tell front window
                    set theTab to active tab
                    set bulkResultText to execute theTab javascript bulkJsCode
                end tell
            end tell

            log nowStr & " : Bulk update result = " & bulkResultText
